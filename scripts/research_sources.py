...
# -*- coding: utf-8 -*-
"""
research_sources.py — 袋井市公式サイトをクロールしてページインベントリを作り、
台帳(topics_master.json)のdraft項目に出典URL候補を自動マッチングする。

工程（「AI下調べ」の機械化・2026-07-02方針に基づく）:
  1) クロール: www.city.fukuroi.shizuoka.jp をトップからBFSで巡回し、
     全HTMLページの URL・<title>・見出し(h1) を data/fukuroi_pages.json に保存。
  ...
  2) マッチング: 各draft項目の (title_iwata + synonyms + 磐田側ラベル語) と
     ...
     上位候補(スコア閾値以上・最大4本)を sources_fukuroi に登録 → status: ai-checked。
...
BASE = 'https://www.city.fukuroi.shizuoka.jp'
LEDGER = Path('data/topics_master.json')
INVENTORY = Path('data/fukuroi_pages.json')
REPORT_DIR = Path('reports')
UA = 'enshu-lifehack-research/1.0 (+https://fukuroi.enshu-lifehack.com/)'
WAIT = 0.5
MAX_PAGES = 6000
...
STOP = set(('袋井 袋井市 磐田 磐田市 について ご案内 案内 一覧 情報 お知らせ ページ ホーム サイト 公式 '

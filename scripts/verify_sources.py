...
# -*- coding: utf-8 -*-
"""
verify_sources.py — 台帳(topics_master.json)の袋井出典URLとfactsを
袋井市公式ページ本文と自動突合する機械検証スクリプト。

方針（2026-07-02確定・全市共通ルール）:
  袋井市役所およびWEBから取れる事実だけを掲載する。
...
検証内容（URL1本ごと）:
  ...
  2) ページ<title>にラベルの主要語が含まれるか（取り違え検知）
     ※ ドメイン直下のトップページ（例: https://www.city.fukuroi.shizuoka.jp/）は
       「〜公式サイト（ホーム）」のような装飾的ラベルが付くだけで<title>と一致しないため対象外
...
def main():
    ...
    for item in targets:
        ...
        if all_url_ok and not fact_ng:
            ...
            ok_items += 1
            lines.append(f"- ✅ machine-verified（URL {len(item.get('sources_fukuroi', []))}本・facts一致 {len(fact_ok)}件）")
        else:
            mismatch_items += 1
        lines += notes + ['']

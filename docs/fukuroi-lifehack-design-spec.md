...
磐田と同じ運用とする：main直コミット、コミットメッセージは日本語、公開に影響する変更はpush前に大石が確認（Claude Codeへの指示文では冒頭に「pushは確認後」と書く。前回の教訓として、**指示文の冒頭と末尾で矛盾する指示を書かない**こと）。
...
磐田と同様にCloudflare Web Analyticsを使用する。ダッシュボード → Web Analytics → Add site で `fukuroi.enshu-lifehack.com` を登録し、発行されたビーコンのトークンを `parts/head-css.html` とは**別の部品 `parts/analytics.html`** として管理する（市ごとにトークンが異なるため。磐田側も同構造へ移行しておくと以後の市で迷わない）。KVは使わない（磐田での障害教訓）。
...
- 市の特徴：高齢化率県内低位・人口増加傾向（子育て系カテゴリの需要が相対的に高い可能性）
